<x-guest-layout>
    <div class="mb-6">
        <h2 class="text-2xl font-semibold text-slate-900 dark:text-gray-100">Reset Password</h2>
        <p class="mt-1 text-sm text-slate-600 dark:text-gray-300">Create a new password to secure your account access.</p>
    </div>
    ...
    <form method="POST" action="{{ route('password.store') }}" class="space-y-4">
        ...
        <x-auth.input-field id="email" name="email" :label="__('Email')" type="email" :value="old('email', $request->email)" autocomplete="username" required autofocus>
            <x-slot:icon>
